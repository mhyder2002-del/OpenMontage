...
| Tools | `subtitle_gen`, `audio_enhance`, `image_selector`, `video_selector`, optional `moneyprinter_turbo` (topic→short only; not a substitute for source selects), `pixabay_music` (free, default), `freesound_music` (free), `music_gen` (ElevenLabs, paid). **Default to `pixabay_music` before reaching for `music_gen`.** | Optional support asset creation |
